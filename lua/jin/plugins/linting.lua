return {
	{
		...
		config = function()
			...
			local function remove_linter_if_missing_config_file(linters, linter_name, config_file_name)
				if linters and linter_in_linters(linters, linter_name) and not file_in_cwd(config_file_name) then
			...
			local function try_linting()
				...
				if linters then
					remove_linter_if_missing_config_file(linters, "eslint", "eslint.config.js")
